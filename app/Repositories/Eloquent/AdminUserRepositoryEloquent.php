...
class AdminUserRepositoryEloquent extends BaseRepository implements AdminUserRepositoryInterface
{
    ...
    public function getALlAdminUsers(){
        $this->model = DB::table('admins')->get();
        // $this->model = DB::table('role_admin')->get();
        return $this->model;
    }
    ...
    */
    public function getAdminUsers($id){
        $this->model = DB::table('admins')->where('id','=',$id)->get();
        // $this->model = DB::table('role_admin')->get();
        return $this->model;
    }

    public function getNameAdmin($id){
        $this->model = DB::table('admins')->where('name','=',$id)->get();
        // $this->model = DB::table('role_admin')->get();
        return $this->model;
    }
    ...
    public function ajaxIndex($request)
    {
        ...
        if ($this->model) {
            foreach ($this->model as $item) {
                ...
                $item->role = ($item->roles->toArray())?$item->roles->toArray()[0]['display_name']:'';//获取关联的角色
            }
        }
