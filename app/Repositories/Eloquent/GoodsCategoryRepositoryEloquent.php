...
use App\Repositories\Contracts\GoodsCategoryRepository as GoodsCategoryRepositoryInterface;

/**
 * Class MenuRepositoryEloquent
 * @package namespace App\Repositories\Eloquent;
 */
class GoodsCategoryRepositoryEloquent extends BaseRepository implements GoodsCategoryRepositoryInterface
{
    /**
...
//                $this->model = $this->model->whereRaw('(status <> 9 and id = ? and (name like ? or short_name like ?))',[$bid, $namelike, $keywords]);
                $this->model = $this->model->whereRaw('(status <> 9 and (name like ? or short_name like ?) and type=0)',[$namelike, $keywords]);
            }else{
//                $this->model = $this->model->whereRaw('(status <> 9 and id = ? and (name = ? or short_name = ?))',[$bid, $nameValue, $kyewordsValue]);
                $this->model = $this->model->whereRaw('(status <> 9 and (name = ? or short_name = ?) and type=0)',[$nameValue, $kyewordsValue]);
            }
        }else{
//            $this->model = $this->model->whereRaw('status <> 9 and id = '.$bid);
            $this->model = $this->model->whereRaw('status <> 9 and type=0');
        }
        ...
        if ($this->model) {
            foreach ($this->model as $item) {
                ...
                $item->button       = $item->getActionButtons('goodscategory', '', true);
            }
        }
